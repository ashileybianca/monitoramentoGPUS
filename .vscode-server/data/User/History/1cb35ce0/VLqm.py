...
def find_peak_values(gpu_data, peak_values, timestamp):
    ...
    for gpu_info in gpu_data:
        ...
        if len(metrics['Power Draw']['Hour'][gpu_index]) <= 12:
            last_hour_pw = metrics['Power Draw']['Hour'][gpu_index][-12:]
            max_pw_hour = max(last_hour_pw)
        ...
        if len(metrics['Power Draw']['Hour'][gpu_index]) <= 288:
            last_day_pw = metrics['Power Draw']['Hour'][gpu_index][-288:]
            max_pw_day = max(last_day_pw)
            if max_pw_day > peak_values[gpu_index]['Power Draw']['Value']:
                peak_values[gpu_index]['Power Draw']['Value'] = max_pw_day
                peak_values[gpu_index]['Power Draw']['Time'] = timestamp

        if len(metrics['Power Draw']['Hour'][gpu_index]) <= 2016:
            last_week_pw = metrics['Power Draw']['Hour'][gpu_index][-2016:]
            max_pw_week = max(last_week_pw)
            if max_pw_week > peak_values[gpu_index]['Power Draw']['Value']:
                peak_values[gpu_index]['Power Draw']['Value'] = max_pw_week
                peak_values[gpu_index]['Power Draw']['Time'] = timestamp
...
def print_peak_values(peak_values, metric_name):
    ...
    for gpu_index, values in peak_values.items():
        if gpu_index != 'LastUpdate':  # Ignorar a chave 'LastUpdate'
            print(f"GPU-{gpu_index}:")
            for period, metrics in values.items():
                print(f"  {period} Peak {metric_name}: {metrics['Value']}W at {metrics['Time']}")
            print()
# Função principal
