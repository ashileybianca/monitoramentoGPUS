...
peak_values_hour = {
    0: {'Power Draw': {'Value': 0, 'Time': None}, 'Fan Speed': {'Value': 0, 'Time': None}, 'Power Draw': {'Value': 0, 'Time': None}},
    1: {'Power Draw': {'Value': 0, 'Time': None}, 'Fan Speed': {'Value': 0, 'Time': None}, 'Power Draw': {'Value': 0, 'Time': None}},
...
def find_peak_values(gpu_data, peak_values, timestamp):
    for gpu_info in gpu_data:
        ...
        if len(metrics['Power Draw']['Hour'][gpu_index]) > 12:
            last_hour_temps = metrics['Power Draw']['Hour'][gpu_index][-12:]
            max_temp_hour = max(last_hour_temps)
            if max_temp_hour > peak_values_hour[gpu_index]['Power Draw']['Value']:
                peak_values_hour[gpu_index]['Power Draw']['Value'] = max_temp_hour
                peak_values_hour[gpu_index]['Power Draw']['Time'] = timestamp

        if len(metrics['Power Draw']['Hour'][gpu_index]) > 288:
            last_day_temps = metrics['Power Draw']['Hour'][gpu_index][-288:]
            max_temp_day = max(last_day_temps)
            if max_temp_day > peak_values_day[gpu_index]['Power Draw']:
                peak_values_day[gpu_index]['Power Draw'] = max_temp_day

        if len(metrics['Power Draw']['Hour'][gpu_index]) > 2016:
            last_week_temps = metrics['Power Draw']['Hour'][gpu_index][-2016:]
            max_temp_week = max(last_week_temps)
            if max_temp_week > peak_values_week[gpu_index]['Power Draw']:
                peak_values_week[gpu_index]['Power Draw'] = max_temp_week
...
def print_peak_values(peak_values):
    ...
    for gpu_index, values in peak_values.items():
        ...
        for period, metrics in values.items():
            print(f"  {period} Peak Power Draw: {metrics['Power Draw']['Value']}°C at {metrics['Power Draw']['Time']}")
        print()
