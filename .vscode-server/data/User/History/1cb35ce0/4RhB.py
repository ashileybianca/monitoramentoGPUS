...
def find_peak_values(gpu_data, peak_values, timestamp):
    ...
    # Verificar se já se passaram 5 minutos desde a última impressão
    if peak_values['LastPrint'] is None or (timestamp - peak_values['LastPrint']) >= timedelta(minutes=5):
        print_temperatures()
        peak_values['LastPrint'] = timestamp
