...
def find_peak_values(gpu_data, peak_values, timestamp):
    for gpu_info in gpu_data:
        ...
        # Verificar pico de hora (última hora)
        if len(metrics['Temperature']['Hour'][gpu_index]) <= 12:
            ...
            if max_temp_hour > peak_values[gpu_index]['Temperature']['Value']:
                peak_values[gpu_index]['Temperature']['Value'] = max_temp_hour
                peak_values[gpu_index]['Temperature']['Time'] = timestamp

        # Verificar pico de dia (últimas 24 horas)
        if len(metrics['Temperature']['Hour'][gpu_index]) <= 288:
            ...
            if max_temp_day > peak_values[gpu_index]['Temperature']['Value']:
                peak_values[gpu_index]['Temperature']['Value'] = max_temp_day
                peak_values[gpu_index]['Temperature']['Time'] = timestamp

        # Verificar pico de semana (últimos 7 dias)
        if len(metrics['Temperature']['Hour'][gpu_index]) <= 2016:
            ...
            if max_temp_week > peak_values[gpu_index]['Temperature']['Value']:
                peak_values[gpu_index]['Temperature']['Value'] = max_temp_week
                peak_values[gpu_index]['Temperature']['Time'] = timestamp
